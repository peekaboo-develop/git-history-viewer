...
test('web server binds loopback and enforces session, host, origin, and method', async (t) => {
  ...
  const page = await request(port, '/', { headers: { Cookie: session } }); assert.equal(page.status, 200); assert.match(String(page.headers['content-security-policy'] ?? ''), /frame-ancestors 'none'/u); assert.match(page.body, /id="loading"/u); assert.match(page.body, /リポジトリを読み込んでいます/u); assert.match(page.body, /Git履歴を読み解く/u); assert.match(page.body, /ローカル・読み取り専用/u); assert.match(page.body, /data-tooltip="MCP設定ガイドを開く"/u);
