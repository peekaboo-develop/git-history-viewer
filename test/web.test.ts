...
test('web server binds loopback and enforces session, host, origin, and method', async (t) => {
  ...
  const page = await request(port, '/', { headers: { Cookie: session } }); assert.equal(page.status, 200); assert.match(String(page.headers['content-security-policy'] ?? ''), /frame-ancestors 'none'/u); assert.match(page.body, /id="loading"/u); assert.match(page.body, /リポジトリを読み込んでいます/u);
  ...
  const styles = await request(port, '/styles.css', { headers: { Cookie: session } }); assert.equal(styles.status, 200); assert.match(styles.body, /loading-progress/u); assert.match(styles.body, /prefers-reduced-motion/u);
