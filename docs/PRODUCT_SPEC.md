...
MCP prompts let the connected host supply a model; the MCP server itself never calls an LLM. Separately, the optional Web adapter can call an explicitly enabled loopback Ollama, OpenAI, Anthropic, or Google profile. It is disabled by default and requires payload preview plus per-request confirmation. Arbitrary compatible endpoints remain out of scope. Official-document recommendations use a fixed registry. Bounded excerpt retrieval and grounded AI transmission are two independent consent actions; the grounded provider input contains excerpt text and citation IDs but no documentation URLs or titles. Remote model aliases can change without notice, so versioned model IDs are recommended where the provider offers them.
