...
MCP prompts let the connected host supply a model; the MCP server itself never calls an LLM. Separately, the optional Web adapter can call an explicitly enabled loopback Ollama, OpenAI, or Anthropic profile. It is disabled by default, sends metadata only, and requires payload preview plus per-request confirmation. Other cloud providers, arbitrary compatible endpoints, and official-document lookup remain out of scope.
