...
The detail pane first recommends fixed official links without network access. Clicking the separate retrieval button connects only to the displayed compiled-registry URLs. The selected destination can infer the detected technology (for example Vue or Vite), but the request sends no repository name, commit, path, message, cookie, credential, or referrer. Retrieval is limited to two pages, one MiB of raw response and 4 KiB of extracted text per page. Extracted text and its commit binding remain only in memory for five minutes.

If AI is enabled, a second, independent preview shows the exact excerpts before another confirmation sends them to the selected provider with commit metadata. The provider receives only bounded excerpt text and stable citation IDs, not documentation URLs, titles, fetch timestamps, document-set IDs, repository names, or commit IDs. Validated generated explanations may be cached, but excerpts and citation targets are not stored in the cache.
