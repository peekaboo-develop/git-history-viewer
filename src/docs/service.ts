...
import { recommendOfficialDocs, type OfficialDocRecommendation } from './official.js';

interface Pending { generation: string; items: OfficialDocRecommendation[]; expiresAt: number }
...
export interface OfficialDocsResult { documents: FetchedOfficialDocument[]; failures: Array<{ id: string; code: string }> }

export class OfficialDocsService {
  private readonly pending = new Map<string, Pending>(); private readonly executions = new Map<string, Promise<OfficialDocsResult>>(); private fetchTail: Promise<void> = Promise.resolve(); private scheduled = 0;
  ...
  async preview(oid: string): Promise<OfficialDocsPreview> {
    ...
    if (requestId) { if (this.pending.size >= 100) this.pending.delete(this.pending.keys().next().value as string); this.pending.set(requestId, { generation, items, expiresAt: now + this.ttlMs }); }
  ...
  async execute(requestId: string): Promise<OfficialDocsResult> {
    ...
    const execution = (async () => {
      ...
      const documents: FetchedOfficialDocument[] = []; const failures: Array<{ id: string; code: string }> = [];
      for (const item of pending.items) {
        try { documents.push(await this.serialFetch(item.id, pending)); }
        catch (error) {
